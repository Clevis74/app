...
import { WaterCalculator } from './Water/WaterCalculator';
    ...
    case 'energy':
      return (
        <EnergyCalculator
          ...
          properties={properties}
          showValues={showValues}
          onAddEnergyBill={onAddEnergyBill}
          onUpdateEnergyBill={onUpdateEnergyBill}
          onDeleteEnergyBill={onDeleteEnergyBill}
        />
      );
    case 'water':
      return (
        <WaterCalculator
          ...
          properties={properties}
          showValues={showValues}
          onAddWaterBill={onAddWaterBill}
          onUpdateWaterBill={onUpdateWaterBill}
          onDeleteWaterBill={onDeleteWaterBill}
        />
      );
    default:
      return <Dashboard summary={summary} properties={properties} transactions={transactions} showValues={showValues} />;
...
function OptimizedApp() {
  ...
  const summary = useMemo(() => {
    return calculateFinancialSummary(properties, transactions);
  }, [properties, transactions]);

  // Memoizar geração de alertas automáticos
  const automaticAlerts = useMemo(() => {
    return generateAutomaticAlerts(properties, tenants, transactions, energyBills, waterBills);
  }, [properties, tenants, transactions, energyBills, waterBills]);

  // Memoizar processamento de transações recorrentes
  const recurringTransactions = useMemo(() => {
    return processRecurringTransactions(transactions);
  }, [transactions]);
